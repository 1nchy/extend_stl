#ifndef _ICY_ITERATOR_HPP_
#define _ICY_ITERATOR_HPP_
...
template <typename _Tp, typename _Pointer = _Tp*, typename _Reference = _Tp&> struct base_iterator {
    ...
    typedef icy::fixed_iterator iterator_category;
    using value_type = _Tp;
    using pointer = _Pointer;
    using reference = _Reference;
    using difference_type = icy::difference_type;
...
struct normal_iterator : public base_iterator <_Tp> {
    typedef icy::random_access_iterator_tag iterator_category;
...
template <typename _Iterator> static difference_type _distance(_Iterator& first, _Iterator& last, icy::random_access_iterator_tag) {
    return last - first;
}
template <typename _Iterator> static difference_type _distance(_Iterator& first, _Iterator& last, icy::bidirectional_iterator_tag) {
...
template <typename _Tp> static void _advance(normal_iterator<_Tp>& _p, difference_type _n, icy::random_access_iterator_tag) {
    _p += _n;
}
template <typename _Iterator> static void _advance(_Iterator& _p, difference_type _n, icy::bidirectional_iterator_tag) {
    if (_n > 0) {
        while (_n--) {
            ++_p;
        }
...
template <typename _Iterator> difference_type distance(_Iterator first, _Iterator last) {
    return _distance(first, last, icy::_A_iterator_category(first));
}
template <typename _Iterator> void advance(_Iterator& _p, difference_type _n) {
    _advance(_p, _n, icy::_A_iterator_category(_p));
}
