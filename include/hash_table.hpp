#ifndef _ICY_HASH_TABLE_HPP_
#define _ICY_HASH_TABLE_HPP_

#include "details/node.hpp"
#include "details/hash_table_policy.hpp"
#include "details/iterator.hpp"
// #include "details/associative_container_helper.hpp"

// #define _HASH_TABLE_DEBUG_
#ifdef _HASH_TABLE_DEBUG_
#include <unordered_set>
#endif // _HASH_TABLE_DEBUG_
...
struct hash_node_iterator {
    ...
    typedef typename std::conditional_t<_Constant, const node_type, node_type> _node_type;
    typedef typename std::conditional_t<_Constant, const value_type, value_type> _value_type;
 ...
 * 
 * 最常规的 unordered_map 与 unordered_set 的声明方式：
 ...
 * 需要牢记的点：在 hash_table 中，_Key 是从 _Value 算出来的，不是外界传进来的
 * 
...
public:
    ...
    size_t bucket_count() const { return _M_in_rehash() ? _rehash_bucket_count : _bucket_count; }
...
protected:
    ...
    */
    ...
    std::pair<iterator, bool> _M_insert(const value_type& _v, std::true_type);
    iterator _M_insert(const value_type& _v, std::false_type);
    size_t _M_erase(const key_type& _k, std::true_type);
    size_t _M_erase(const key_type& _k, std::false_type);
    iterator _M_update(const value_type& _v, std::true_type);
    iterator _M_update(const value_type& _v, std::false_type);

    /// rehash policy
    std::pair<bool, size_t> _M_need_rehash(size_t _ins = 1) const { return this->_rehash_policy.need_rehash(_bucket_count, _element_count, _ins); }
    bool _M_in_rehash() const { return this->_rehash_policy._in_rehash; }
    virtual void _M_start_rehash(size_t _next_bkt);
    virtual void _M_finish_rehash();
    virtual rehash_policy::status _M_step_rehash(size_t _step = 1);
    /**
...
-> size_t {
    ...
    const hash_code _c = this->_M_hash_code(_k);
    auto erase_in_given_bucket = [&](const bucket_index& _i) -> size_t {
        if (!this->_M_valid_bucket_index(_i)) return 0;
...
-> int {
    ...
    */
    ...
    std::decay_t<key_type> _last_value;
    if (!_M_in_rehash()) {
...
-> rehash_policy::status {
    ...
    if (_rehash_policy._cur_process.first != 0) { return rehash_policy::status::__FAILED__; }
    while (_step--) {
        ...
        if (_hint == nullptr || _hint == _M_end()) {
            return rehash_policy::status::__COMPLETED__;
        }
...
-> void {
    ...
    if (this->_M_in_rehash()) {
        ...
        if (_ret == rehash_policy::status::__COMPLETED__) {
            this->_M_finish_rehash();
        }
    }
};
